...
namespace CartoonFX
{
    ...
    public class CFXR_ParticleText : MonoBehaviour
    {
        ...
        [Header("Loop")]
        [SerializeField] private bool loopTextParticles = false;
...
#pragma warning restore 0649
        ...
        void Awake()
        {
            ...
            if (loopTextParticles)
            {
                loopCoroutine = StartCoroutine(LoopParticles());
            }
        }
        ...
        private IEnumerator LoopParticles()
        {
            while (loopTextParticles)
            {
                var ps = GetComponent<ParticleSystem>();
                ps.Clear(true);
                ps.Play(true);

                yield return new WaitForSeconds(ps.main.duration + loopDelay);
            }
        }
    }
